...
class Grid:
    def __init__(self, grid_dict: List[List[str]]) -> None:
        ...
        self.Rmin = 0
        self.Rmax = len(grid_dict)
        self.Cmin = 0
        self.Cmax = len(grid_dict[0])

        for r in range(self.Rmin, self.Rmax):
    ...
    def copy(self):
        new_grid = Grid([[' ' for _ in range(self.Cmin, self.Cmax)] for _ in range(self.Rmin, self.Rmax + 1)])
        new_grid.grid = self.grid.copy()
        return new_grid

    def range(self):
        return Pos(self.Rmin, self.Cmin).range(Pos(self.Rmax - 1, self.Cmax - 1))
    ...
    def get_neigh_pos(self, pos):
        r, c = pos
        dirs = [Pos(i + r, j + c) for i in [-1, 0, 1] for j in [-1, 0, 1] if not (i+r == r and j+c == c)]
        return [el for el in dirs if el in self]

    def get_neigh(self, pos):
        nkeys = self.get_neigh_pos(pos)
        return [self[el] for el in nkeys]
    ...
    def to_string(self) -> str:
        result = ""
        for r in range(self.Rmin, self.Rmax):
            row_str = ''.join(self.grid.get(Pos(r, c), ' ') for c in range(self.Cmin, self.Cmax))
            result += row_str + "\n"
        return result

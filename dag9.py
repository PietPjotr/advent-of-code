...
def bfs(basins, a, i, j):
    m = a
    k = m[i][j]
    if i > 0 and m[i-1][j] > k and m[i-1][j] < 9:
        basins.append(m[i-1][j])
        m[i][j] = 9
        bfs(basins, a, i-1, j)

    if j > 0 and m[i][j-1] > k and m[i][j-1] < 9:
        basins.append(m[i][j-1])
        m[i][j] = 9
        bfs(basins, a, i, j - 1)

    if i < len(m)-1 and m[i+1][j] > k and m[i+1][j] < 9:
        basins.append(m[i+1][j])
        m[i][j] = 9
        bfs(basins, a, i+1, j)

    if j < len(m[i])-1 and m[i][j+1] > k and m[i][j+1] < 9:
        basins.append(m[i][j+1])
        m[i][j] = 9
        bfs(basins, a, i, j+1)
